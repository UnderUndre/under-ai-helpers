...
| `CLAUDE.md` | Root persona/operating instructions. Composed from persona Foundation + coding Foundation via `<!-- HELPERS:REF -->` markers + hand-maintained bespoke sections (§2, §4). The two Foundations are single-source for all four tools; bespoke sections (MCP, Agent Routing, Intent Routing, Quick Ref, Project Ref, Ultrathink, Context Mgmt) are edited directly in CLAUDE.md. |
