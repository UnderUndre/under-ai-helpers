...
- **Decision** *(revised 2026-06-27, supersedes the two-salt design in response to cross-AI review claude.md F6 / gemini bot PR#7)*: PBKDF2 with ≥600,000 iterations (OWASP 2023 guidance for PBKDF2-HMAC-SHA256) and a single per-profile random salt (≥16 bytes) for the AES-256-GCM encryption-key derivation. **No separate passphrase-verification hash or verification-hash salt is stored.** Passphrase correctness is verified solely via the GCM authentication tag — decrypt failure (tag invalid) = wrong passphrase. The passphrase is requested per push/pull operation, never cached in the long-lived MCP server process; the derived AES-256-GCM key is zeroed from memory immediately after use. The iteration count is stored per-profile so old profiles can be migrated if the floor rises; the KDF parameters also travel in the sync-file cleartext header so a fresh machine can bootstrap a first pull (see profile-sync.md envelope contract).
- **Rationale**: The threat model (sync file at rest on a user-controlled cloud drive / USB) means an attacker who obtains the file can brute-force the passphrase offline. Default PBKDF2 iteration counts (e.g., 100k) are brute-forceable on modern GPUs; 600k raises the cost into the impractical range for a passphrase of reasonable entropy. **The previous two-salt design stored a PBKDF2 verification hash locally "for verification only," but AES-256-GCM already authenticates the derived key via its tag — the contract's own `WRONG_PASSPHRASE` error is defined as "GCM tag invalid on the first derived key." The stored hash therefore verified nothing GCM does not, and it planted an offline brute-force oracle in the local `~/.underboard/data.db` that the original threat model (attacker has the remote sync file) never intended to expose. The original rationale ("two distinct salts prevent a single salt compromise from weakening both the verification hash and the encryption key") is moot once the verification hash is removed — there is only one derived key and one salt. Single-user local-first apps on laptops without full-disk encryption face a realistic local-DB-leak vector (theft, cloud backup, malware) that makes any stored passphrase-derived material a liability, especially under passphrase reuse.** Per-operation passphrase entry and key zeroing limit the window of memory exposure in the long-lived MCP server process.
- **Alternatives considered**: (a) Argon2id — preferred over PBKDF2 in modern guidance, but Node.js `crypto` does not ship it built-in; adding `argon2` npm package would increase audit surface (rejected per the Dependencies decision above); (b) a single shared salt with a stored verification hash — the original design; rejected as it creates the brute-force oracle above; (c) retain the verification hash but store it only in process memory, never on disk — defeats the "fast pre-decrypt fail" purpose, since PBKDF2 dominates cost and GCM-tag check is microseconds; the in-memory copy would have to be re-derived from the passphrase on every operation, at which point the GCM tag check itself is the verification.
...
| D007 | Signal retention default = 30 days (most privacy-protective non-zero) | Balances inference quality with privacy (FR-015; cross-ref: data-model.md:19 `retention_days DEFAULT 30`, T019) |
...
| D009 | Sync: PBKDF2 ≥600k iterations, single per-profile salt for encryption key (verification hash dropped — GCM tag is sole verifier), per-operation passphrase, key zeroed | Raises offline-brute-force cost into impractical range; limits memory exposure; removes offline brute-force oracle from local DB (FR-023, review F3; revised 2026-06-27 per cross-AI review claude.md F6 + gemini PR#7) |
