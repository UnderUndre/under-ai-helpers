...
- [ ] T000 [SETUP] Create implementation branch `009-configurable-endpoints` from `main` (NOT from `specs/009-configurable-endpoints`). This branch holds all code changes per Principle IX. Planning branch `specs/009-configurable-endpoints` retains spec-only artifacts.
- [ ] T001 [SETUP] Read `packages/underboard/src/cli/config.ts` and `packages/underboard/src/cli/index.ts`, confirm `UnderboardConfig` interface location and current shape, document the existing fields in a research.md appendendum for traceability.
- [ ] T002 [SETUP] Define the new configuration interfaces (`honcho`, `embedding`, `llm` shapes) in `packages/underboard/src/cli/config.ts`.
...
- [ ] T006 [BE] [US1] Update `packages/underboard/src/memory-backend/backend-factory.ts` to instantiate `HonchoClient` with the resolved endpoint, token, and timeout.
- [ ] T007 [BE] [US1] Verify existing BM25 fallback in `packages/underboard/src/memory-backend/honcho.ts` (HonchoBackend.recall already catches and falls back to `this.local.recall` per F14). Wire `honcho.timeout_ms` config into `HonchoClient.fetchWithTimeout`. Do NOT add fallback logic to `honcho-client.ts` (wrong layer — client throws, backend degrades). Re-scope: "verify + wire config," not "implement."
- [ ] T008 [BE] [US1] Write unit and integration tests in `packages/underboard/tests/` to verify Honcho custom backend endpoint, token, and timeout resolution and graceful fallback behavior (both recall and write paths).
- [ ] T008a [BE] [US1] Update `memory_write` tool response to include `synced: boolean` flag. On Honcho timeout, set `synced: false` and persist locally + enqueue for sync. Implement rate-limited degradation warnings (max 1 per 5 min per op type after first).
...
- [ ] T011 [BE] [US2] Write unit and integration tests to verify embedding model custom path loading, fallback on missing path (`disabled`), error on missing file (`failed`), and CLI argument precedence.
- [ ] T011a [BE] [US2] Verify `memory_recall` tool response already includes `embedding_status` field (existing `"ready" | "lexical_only"` union, per FR-010). No schema change needed — just confirm the field propagates correctly when embedding subsystem is in `disabled` or `failed` internal state (both map to `"lexical_only"` response).
...
- [ ] T012 [BE] [US3] Update LLM/dialectic recall tools to parse LLM endpoint, API key, and model from resolved configuration.
- [ ] T013 [BE] [US3] Add validation and error logging for LLM calls (e.g. error on missing API key when dialectic recall is invoked).
...
- [ ] T014 [OPS] Document configuration options and examples in `packages/underboard/README.md`.
- [ ] T015 [OPS] Validate that `underboard start` boots successfully under STDIO mode and prints configuration details to `stderr` with sensitive values redacted.
