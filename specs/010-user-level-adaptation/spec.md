...
- **FR-023**: System MUST NOT cache the sync passphrase in the long-lived MCP server process memory. The passphrase MUST be requested per push/pull operation, the derived encryption key MUST be zeroed from memory immediately after use, and PBKDF2 key derivation MUST use at least 600,000 iterations with a per-profile random salt. The salt MUST be stored alongside the profile. Passphrase correctness MUST be verified solely via the AES-256-GCM authentication tag (decrypt failure = wrong passphrase); the system MUST NOT store a separate passphrase-derived verification hash, as that would create an offline brute-force oracle in the local database without authenticating anything the GCM tag does not already authenticate (see research.md ADR on FR-023 evolution).
